...
<div class="container-fluid">
  <div class="row" id="mygallery" >
	<?php
		$i=0;
		foreach($idxs as $idx) {
			if ($i == TOTAL_ITEM_FIRST_LOAD) {
				break;
			}
			$i++;
			$file = $files[$idx];
			echo '<a href="' . GALLERY_ROOT . $file . '"><img class="img-fluid lazy" src="' . GALLERY_ROOT . $file . '" /></a>';
		}
	?>
  </div>
</div>
...
<script>
	data.imgs = <?=json_encode($files)?>;
	data.idxs = <?=json_encode($idxs)?>;
	data.offset = <?=$i?>;
	...
	$(function() {
		...
		$(window).scroll(function() {
			if($(window).scrollTop() + $(window).height() == $(document).height()) {
				var nextOffset = data.offset + 20;
				for (; data.offset < nextOffset; data.offset++) {
					var nextIdx = data.idxs[data.offset];
					var imgFileName = data.imgs[nextIdx];
